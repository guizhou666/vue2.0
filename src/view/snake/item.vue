<template>
    <ul class="f-snake-timeline">
        <li class="flex flex-align-start flex-justify-between f-snake-timeline-rows" v-for="(items, index) in timeLineList"
            :key="index">
            <template v-for="(subItem, subIdx) in items">
                <div class="f-snake-timeline-items" :class="{ hidden: subItem._hidden }" :key="subIdx">
                    ...
                    <div v-if="subItem._isBottomIcon" class="arrows-bottom-wrap">
                        <slot name="bottomArrows" v-bind:data="subItem">
                            <img class="bottom-img" src="../../assets/images/chevronsdownshuangxiajiantou.png" alt />
                        </slot>
                    </div>
                </div>
                <div v-if="subIdx !== items.length - 1 && subItem._odd" class="arrows-wrap"
                    :class="{ hidden: items[subIdx + 1] && items[subIdx + 1]._hidden }" :key="subIdx + '1'">
                    <slot name="rightArrows" v-bind:data="subItem">
                        <img class="arrow-img" src="../../assets/images/arrow.png" alt />
                    </slot>
                </div>
                <div v-if="subIdx !== items.length - 1 && !subItem._odd" class="arrows-wrap"
                    :class="{ hidden: items[subIdx] && items[subIdx]._hidden }" :key="subIdx + '2'">
                    <slot name="leftArrows" v-bind:data="subItem">
                        <img class="arrow-img" :class="{ rotate180: !subItem._odd }" src="../../assets/images/arrow.png"
                            alt />
                    </slot>
                </div>
            </template>
